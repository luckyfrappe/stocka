...
class Order(models.Model):
    ...
    def update_total(self):
        """
        Updates the grand total and delivery costs whenever a line item is
        modified.
        """
        ...
        if (
            ...
            all(
                item.purchase_type not in [
                PurchaseType.BUYOUT, PurchaseType.EXTEND
...
class OrderLineItem(models.Model):
    ...
    def price_each(self):
        ...
        if self.quantity > 0:
            return (self.lineitem_total / self.quantity).quantize(
                Decimal('0.00')
            )
        return Decimal('0.00')
    ...
    def save(self, *args, **kwargs):
        ...
        # 2. Logic for Preowned (using settings rate)
            ...
            base_price = (self.product.retail_price * discount).quantize(
                Decimal('0.00')
            )

        # 3. Logic for Buyout
        elif self.purchase_type == PurchaseType.BUYOUT:
            # Look for the current active subscription
            ...
            if active_sub:
                sunk_cost = (
                    self.product.price_per_week * active_sub.duration_weeks
                )
